...
class CreateProfile extends Component {
	...
	componentWillReceiveProps(nextProps){  // `nextProps` includes any new properties
		...
		if(nextProps.profile.profile){  // check if profile has come in from state
			...
			profile.company = !isEmpty(profile.company) ? profile.company : '';  // if profile.company is there, then use profile.company. otherwise it is to be an empty string.
...
export default connect(mapStateToProps, { createProfile, getCurrentProfile })(withRouter(CreateProfile));  // must wrap the component we are exporting with `withRouter`